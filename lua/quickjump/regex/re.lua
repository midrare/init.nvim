  ...
  for n = 1, path.nGroups * 3, 3 do
    if path[n + 1] and path[n + 2] then
      print(
        ...
        (n - 1) / 3 + 1,
        '[' .. path[n + 1] .. ':' .. path[n + 2] .. ']',
        self.input:sub(path[n + 1], path[n + 2])
      )
    end
  end
end
...
function compare(path1, path2)
  ...
  for n = 1, nGroups * 3, 3 do
    ...
    if path1[n + 2] then
      len1 = path1[n + 2] - path1[n + 1]
    end
    if path2[n + 2] then
      len2 = path2[n + 2] - path2[n + 1]
    end
